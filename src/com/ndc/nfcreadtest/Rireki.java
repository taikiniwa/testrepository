...
// add start
    public RirekiMap toMap() {
    	...
    	map.setTermId(this.termId);		// 端末種
    	map.setProcId(this.procId);		// 処理
    	map.setYear(this.year);			// 年
    	map.setMonth(this.month);		// 月
    	map.setDay(this.day);			// 日
    	map.setKind(this.kind);			// 種別
    	map.setRemain(this.remain);		// 残高
    	map.setSeqNo(this.seqNo);		// 連番
    	map.setReasion(this.reasion);	// リージョン
    	map.setPictId(this.pictId);		// 画像ID
